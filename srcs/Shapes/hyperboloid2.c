...
/*   Updated: 2023/05/14 00:35:04 by albaud           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../header.h"

void	hyperboloid2_cap(t_ray *ray, double *sols)
{
	double	denom;
	t_v3	pt1;
	t_v3	pt2;
	...
	if (denom < 0)
		return ;
	pt1 = v_rm(&(t_v3){0, 0, 2}, &ray->origin);
	pt2 = v_rm(&(t_v3){0, 0, -2}, &ray->origin);
	...
	pt1 = v_rm(&pt1, &(t_v3){0, 0, 2});
	pt2 = v_rm(&pt2, &(t_v3){0, 0, -2});
	if (v_dotp(&pt1, &pt1) < 3)
		sols[2] = e.x1;
	if (v_dotp(&pt2, &pt2) < 3)
		sols[3] = e.x2;
...
// void	hyperboloid2_normal(t_hit *hit)
// {
